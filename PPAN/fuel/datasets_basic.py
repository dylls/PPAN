...
class BasicDataset(object):
    ...
    def get_data(self, pickle_path):
        ...
        with open(pickle_path + '/filenames.pickle', 'rb') as f:
            if sys.version_info.major > 2:
                self.filenames = pickle.load(f, encoding="bytes")
            else:
    ...
    def readCaptions(self, filenames, class_id):
        name = filenames
        if not 'birds' in self.workdir:
            if name.decode('utf8').find('jpg/') != -1:  # flowers dataset
                class_name = 'class_{0:05d}/'.format(class_id)
                name = name.decode('utf8').replace('jpg/', class_name)
    ...
    def __next__(self):
        ...
        ret_list.append(class_id_array)
        ret_list.append(wrong_class_id_array)
    ...
    def next_batch_test(self, max_captions=1):
        ...
        sent_ix = np.random.randint(0, embedding_num)
        print(sent_ix)
        batch = sampled_embeddings[:, sent_ix, :]
        sampled_embeddings_batchs.append(batch)
        # return [sampled_images, sampled_embeddings_batchs, sampled_captions,
        #         self.saveIDs[start:end], self.class_id[start:end]]

        return [sampled_images, sampled_embeddings_batchs, sampled_captions,
                self.saveIDs[start:end], self.class_id[start:end], sent_ix]

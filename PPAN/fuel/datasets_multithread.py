...
class Dataset(object):
    def __init__(self, workdir, img_size, batch_size, n_embed, mode='train'):
        ...
        self.embedding_filename = '/char-CNN-RNN-embeddings_class.pickle'
        ...
        self.get_data(os.path.join(self.workdir, mode+'_class'))
    ...
    def get_data(self, data_dir):
        ...
        with open(data_dir + '/filenames_class.pickle', 'rb') as f:
            self.filenames = pickle.load(f)
            # print('list_filenames: ', len(self.filenames))

        with open(data_dir + '/captions_class.pickle', 'rb') as f:
        ...
        with open(data_dir + '/class_info.pickle', 'rb') as f:
            ...
            self.class_id = np.array(class_id)  #one-hot(num, 91)

        assert len(self.filenames) == len(self.captions) == len(self.embeddings) == self.class_id.shape[0], 'dataset !='
    ...
    def __getitem__(self, index):
        ...
        class_id_array = np.array(class_id)
        wrong_class_id = [self.class_id[i] for i in fake_ids]
        wrong_class_id_array = np.array(wrong_class_id)
        ret_list.append(class_id_array.squeeze())
        ret_list.append(wrong_class_id_array.squeeze())
